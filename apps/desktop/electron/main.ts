...
const APP_NAME = process.env.HERMES_DESKTOP_APP_NAME || 'Hermes [fork]'
...
// Electron handles rejected ipcMain handlers internally, so they never reach
// the process-level crash listeners above. Record them before rethrowing so
// DevTools retains its normal error reporting while desktop.log has evidence.
